...
#chnage the directory to where the Simulation 5 .py file is located
...
#building the multiplicative input signal
...
for i in range(N_trials):
    ...
    for j in range(time):
        ...
        ### G layer
        g_exc = (np.dot(W_go,rnn_E) - np.dot(W_inh,N) - np.dot(W_g_inh,G))*rhythm[j]
...
fig = plt.figure()
plt.title('Temporal compositionality',fontsize=25, y=1.05)
...
plt.axvline(x=new_tempo[0], color='b',linewidth=3)
plt.axvline(x=new_tempo[1], color='r',linewidth=3)
plt.axvline(x=new_tempo[2], color='m',linewidth=3)
plt.axvline(x=new_tempo[3], color='g',linewidth=3)
plt.axvline(x=new_tempo[4], color='c',linewidth=3)
plt.axvline(x=new_tempo[5], color='y',linewidth=3)
plt.xlabel('Time (ms)', fontsize=20)
plt.ylabel('A nodes activation', fontsize=20)
plt.legend()
plt.savefig('Rhythm compo.png', dpi=500, bbox_inches='tight')
plt.show()

fig = plt.figure()
plt.title('Multiplicative tempo signal',fontsize=25, y=1.05)
plt.plot(rhythm, linewidth = 2)
plt.xlabel('Time (ms)', fontsize=20)
plt.ylabel('Signal value', fontsize=20)
plt.savefig('Multi Signal.png', dpi=500, bbox_inches='tight')
plt.show()
