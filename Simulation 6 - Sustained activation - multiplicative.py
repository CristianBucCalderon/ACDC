...
gain_to_N = np.ones(time)
gain = 0.09
gain_to_N[int(times[1]):int(times[2])] = gain
...
for i in range(N_trials):
    ...
    for j in range(time):
        ...
        ### G layer
        g_exc = (np.dot(W_go,rnn_E) - np.dot(W_inh,N) - np.dot(W_g_inh,G)) #if single gain then gain_to_G
...
fig = plt.figure()
plt.title('Via multiplicative signal',fontsize=25, y=1.05)
...
plt.legend()
plt.savefig('Sustained - multiplicative.png', dpi=500, bbox_inches='tight')
plt.show()
